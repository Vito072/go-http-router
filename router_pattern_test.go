...
func TestRouterPatternNamedParameter(t *testing.T) {
	...
	router.GET("/products/:id/items/:id", func(w http.ResponseWriter, r *http.Request, p httprouter.Params) {
		idProducts := p.ByName("id")
		idItems := p.ByName("id")
		text := "Product " + idProducts + " Item " + idItems
		fmt.Fprint(w, text)
	})
